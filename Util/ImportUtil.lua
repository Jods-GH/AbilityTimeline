...
function ImportUtil:ParseJSONFormat(jsonText)
    ...
    local encounterId = tonumber(jsonText:match('"encounterId"%s*:%s*(%d+)')) or tonumber(jsonText:match('encounterId%s*:%s*(%d+)'))
    if encounterId then
...
function ImportUtil:DoesTagApplyToPlayer(tag)
    ...
    local playerSpecID, playerSpec, _,  _,  playerRole,  _,  _,  _,  _,  _ = C_SpecializationInfo.GetSpecializationInfo(C_SpecializationInfo.GetSpecialization())
    local _, subgroup
